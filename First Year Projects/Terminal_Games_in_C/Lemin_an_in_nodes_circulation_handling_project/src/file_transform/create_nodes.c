...
list_data_t *node)
{
    ...
    node->node->best_path = NULL;
    node->node->prev = NULL;
    node->node->was_visited = 0;
    node->node->g_cost = -1;
...
list_data_t *create_classic_node(char *line)
{
    list_data_t *node = malloc(sizeof(list_data_t));
