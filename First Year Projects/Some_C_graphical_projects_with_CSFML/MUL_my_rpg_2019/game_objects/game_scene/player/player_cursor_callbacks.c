...
, struct player_cursor *player_c, list_t *own_scene)
{
    ...
    player_c->inventory->update(prog_stat, player_c->inventory, own_scene);
    sfView_setCenter(player_c->view, player_c->sprite_pos);
    sfRenderWindow_setView(prog_stat->window, player_c->view);
}
...
, struct player_cursor *player_c, list_t *own_scene)
{
    ...
    player_c->inventory->render(prog_stat, player_c->inventory, own_scene);
}
...
, struct player_cursor *player_c, list_t *own_scene)
{
    ...
    sfView_destroy(player_c->view);
    player_c->inventory->destroy(prog_stat, player_c->inventory, own_scene);
    free(player_c->rect);
    free(player_c);
    player_c = NULL;
}